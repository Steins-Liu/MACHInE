...
from model.models import build_transformer
...
class PlotSteeringLoss(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        plt.savefig("transformer_steering_loss_history.png")
...
class PlotThrottleLoss(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        plt.xlabel('epoch')
        plt.legend(['train', 'val'], loc='upper left')
        plt.savefig("transformer_throttle_loss_history.png")
...
class Generator(tf.keras.utils.Sequence) :
  ...
  def __getitem__(self, idx) :
    ...
    for a in range(len(batch_image)):
         batch_img.append((cv2.resize(cv2.imread(batch_image[a][0]), (162,162))[:, :]))
...
def Predict_Image_Generator(image_filenames):
    ...
    for a in range(len(image_filenames)):
         img.append(np.stack((cv2.resize(cv2.imread(image_filenames[a][0]), (162,162))[:, :])))
    return np.stack(img)
...
def main(*args, **kwargs):
    ...
    saved_weight_name = './transformer.h5'
    saved_file_name = './transformer.hdf5'.format(
    ...
    for a in range(len(val_images)):
         img_val.append((cv2.resize(cv2.imread(val_images[a][0]),(162,162))[:, :]))   
