...
from data_utils.data_processor import load_3d_sensor_dataset
...
class PlotSteeringLoss(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        plt.savefig("lrcn_v1_steering_loss_history.png")
...
class PlotThrottleLoss(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        plt.xlabel('epoch')
        plt.legend(['train', 'val'], loc='upper left')
        plt.savefig("lrcn_v1_throttle_loss_history.png")
...
class Generator(keras.utils.Sequence) :
  ...
  def __getitem__(self, idx) :
    ...
    for a in range(len(batch_image)):
         batch_img.append((np.stack([(cv2.imread(batch_image[a][0])[28:, :]).astype(np.float16), (cv2.imread(batch_image[a][1])[28:, :]).astype(np.float16)])))
...
def Predict_Image_Generator(image_filenames):
    ...
    for a in range(len(image_filenames)):
         img.append(np.stack((np.stack([(cv2.imread(image_filenames[a][0])[28:, :]).astype(np.float16), (cv2.imread(image_filenames[a][1])[28:, :]).astype(np.float16)]))))
    return np.stack(img)
...
def main(*args, **kwargs):
    ...
    saved_weight_name = './lrcn_v1.h5'
    saved_file_name = './lrcn_v1.hdf5'.format(
    ...
    train_images, train_sensors, val_images, val_sensors, test_images, test_sensors, train_steering, val_steering, test_steering, train_throttle, val_throttle, test_throttle = load_3d_sensor_dataset(
...
#######################################Steering & Throttle############################################
    ...
    print('Throttle R^2: ' + str(R2_test))

    t = np.arange(0, len(test_steering[:100]), 1)	
    plt.plot(t, test_throttle[:100], 'r--', t, model_throttle_test[:100], 'g--')
    plt.plot(t, test_steering[:100], 'y--', t, model_steering_test[:100], 'b--')
    plt.show()
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str, default='/mnt/c334c9bc-7ae4-4ea7-84fb-6b8f5595aea2/grc_2/final/final.csv'
    )
    parser.add_argument(
