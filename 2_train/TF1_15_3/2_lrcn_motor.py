...
class Generator(keras.utils.Sequence) :
  ...
  def __getitem__(self, idx) :
    ...
    for a in range(len(batch_image)):
         batch_img.append((np.stack([(cv2.imread(batch_image[a][0])[22:, :]).astype(np.float16)])))
...
def Predict_Image_Generator(image_filenames):
    ...
    for a in range(len(image_filenames)):
         img.append(np.stack((np.stack([(cv2.imread(image_filenames[a][0])[22:, :]).astype(np.float16)]))))
    return np.stack(img)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str, default='/mnt/c334c9bc-7ae4-4ea7-84fb-6b8f5595aea2/homecircuit2/final/final.csv'
    )
    parser.add_argument(
